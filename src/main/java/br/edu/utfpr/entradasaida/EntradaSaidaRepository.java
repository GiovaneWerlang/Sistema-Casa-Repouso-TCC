...
public class EntradaSaidaRepository extends CrudRepository<EntradaSaidaModel> {
    public List<DadoDTO> getDadosEntradaSaidaResidentesDash(){
        return find("select r.nome as label, count(e.id) as data from EntradaSaidaModel e join ResidenteModel r on r.id = e.residente group by r.nome").project(DadoDTO.class).list();
    }
}
